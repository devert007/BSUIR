...
class Memory {
    ...
    pthread_mutex_t lock; // для синхронизации нашей памяти

    const intptr_t page_size; //размер страницы памяти

    long page_count = 0;

    void** pages = (void**) calloc(0, sizeof(void*));//массив указателей на наши страницы

    std::map<intptr_t , intptr_t> allocated;//карта которая отслеживает выделенные области памяти
    ...
    void* alloc_page() {
      //выделяем новую страницу памяти, увеличивая массив страниц и используя sbrk для выделения памяти 
...
public:
    ...
    void* alloc(intptr_t size) {
        ...
        if (!succ) {
            alloc_page();//создаем новую страницу 
            find_free_space(size, page, offset);
        }

        void* ptr = (void*)((intptr_t)pages[page] + offset);//указатель на начало выделенного блока
        allocated[(intptr_t)pages[page] + offset] = (intptr_t)pages[page] + size + offset - 1;// вструктуру allocated записываем информацию о выделенном блоке
